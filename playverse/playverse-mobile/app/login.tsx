...
export default function LoginScreen({ onSwitchToRegister, showHeader = true }: { onSwitchToRegister?: () => void; showHeader?: boolean; }) {
  const cs = useColorScheme();
  const colors = Colors[cs ?? 'light'];
  const router = useRouter();
  const [email, setEmail] = useState(''), [password, setPassword] = useState(''), [rememberMe, setRememberMe] = useState(false);
          ...
          <View style={[styles.formCard, { backgroundColor: colors.cardBackground, borderColor: colors.secondary }]}>
            ...
            <View style={styles.checkboxRow}>
              <TouchableOpacity style={styles.checkboxContainer} onPress={() => setRememberMe(!rememberMe)}>
                <View style={[styles.checkbox, { borderColor: rememberMe ? colors.secondary : colors.gray }]}>{rememberMe && <Ionicons name="checkmark" size={16} color={colors.secondary} />}</View>
                <Text style={{ color: colors.white }}>Recordarme</Text>
              </TouchableOpacity>
              <TouchableOpacity><Text style={{ color: colors.secondary, fontWeight: '600' }}>¿Olvidaste tu contraseña?</Text></TouchableOpacity>
            </View>
            <TouchableOpacity style={[styles.loginButton, { backgroundColor: colors.secondary }]} onPress={() => router.replace('/(tabs)')}>
            ...
            <View style={{ alignItems: 'center' }}>
              <Text style={{ color: colors.white }}>¿No tienes una cuenta? <Text style={{ color: colors.secondary, fontWeight: 'bold' }} onPress={onSwitchToRegister || (() => router.replace('/register'))}>Regístrate aquí</Text></Text>
            </View>
          </View>
        </View>
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  checkboxRow:{flexDirection:'row',justifyContent:'space-between',alignItems:'center',marginBottom:24},
  checkboxContainer:{flexDirection:'row',alignItems:'center'}, checkbox:{width:20,height:20,borderWidth:2,borderRadius:4,justifyContent:'center',alignItems:'center',marginRight:8},
  loginButton:{flexDirection:'row',alignItems:'center',justifyContent:'center',paddingVertical:16,borderRadius:12,marginBottom:20},
  loginButtonText:{fontSize:18,fontWeight:'bold',marginLeft:8}
});
