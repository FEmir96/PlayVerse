...
import {
  RefreshControl, ScrollView, StyleSheet, Text, View, useWindowDimensions, Pressable, Image,
} from 'react-native';
        ...
        <View style={styles.grid}>
          {visible.map((row: any, i: number) => {
            ...
            return (
              <View
                key={String(gameId)}
                style={{ width: cardWidth, marginRight: i % columns !== columns - 1 ? GAP : 0, marginBottom: GAP }}
              >
                <GameCard
                  game={{
                    id: String(gameId),
